 ...
 * Obsidian destroys the WritingManuscript instance and creates a new one when
 ...
 */
export class WritingManuscript extends ItemView {
    ...
    private _plainText = WritingManuscript.loadPlainTextPref();
    ...
    private static loadPlainTextPref(): boolean {
        try {
            const v = window.localStorage.getItem(WritingManuscript.PLAIN_TEXT_PREF_KEY);
    ...
    private persistPlainTextPref(): void {
        try {
            window.localStorage.setItem(
                WritingManuscript.PLAIN_TEXT_PREF_KEY,
                this._plainText ? '1' : '0',
            );
        } catch {
    ...
    getIcon(): string {
        ...
        if (typeof applyMobileClass === 'function') {
    ...
    async onClose(): Promise<void> {
        ...
        // variable so the next WritingManuscript instance can restore it.
    ...
    // Obsidian destroys this WritingManuscript instance and creates a new one on
