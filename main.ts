...
import { addIcon, App, ButtonComponent, DropdownComponent, FuzzySuggestModal, ItemView, Menu, Modal, Notice, Platform, Plugin, Setting, TFile, TextComponent, ToggleComponent, WorkspaceLeaf, normalizePath, parseYaml, setIcon } from 'obsidian';
...
export default class SceneCardsPlugin extends Plugin {
    ...
    private readonly workspaceLayoutStorageKey = 'storyline-workspace-routes-layouts';
    private defaultWorkspaceLayout: Record<string, unknown> | null = null;
    private workspaceLayouts: Record<string, Record<string, unknown>> = {};
    private currentWorkspaceRoute = 'home';
    ...
    private cloneLayout(layout: Record<string, unknown> | null | undefined): Record<string, unknown> | null {
    ...
    private safeCurrentWorkspaceLayout(): Record<string, unknown> | null {
        try {
            return this.cloneLayout(this.app.workspace.getLayout()) ?? null;
        } catch {
            return null;
        }
    }

    private loadWorkspaceLayouts(): void {
        try {
            const payload = window.localStorage.getItem(this.workspaceLayoutStorageKey);
            if (!payload) {
                this.workspaceLayouts = {};
                return;
            }
            this.workspaceLayouts = JSON.parse(payload) as Record<string, Record<string, unknown>>;
        } catch {
            this.workspaceLayouts = {};
        }
    }

    private persistWorkspaceLayouts(): void {
        try {
            window.localStorage.setItem(this.workspaceLayoutStorageKey, JSON.stringify(this.workspaceLayouts));
        } catch {
            // localStorage may be unavailable in some contexts.
        }
    }

    private showWorkspaceRibbonMenu(evt: MouseEvent): void {
        const menu = new Menu();
        const items = [
            { label: 'Home', icon: 'home', route: 'home' },
            { label: 'Dashboard', icon: 'layout-dashboard', route: 'dashboard' },
            { label: 'Connections', icon: 'link', route: 'connections' },
            { label: 'Writing', icon: 'pen-line', route: 'writing' },
            { label: 'Worldbuilding', icon: 'globe-2', route: 'worldbuilding' },
            { label: 'Databases', icon: 'database', route: 'databases' },
        ];
        ...
        for (const item of items) {
            menu.addItem((entry) => {
                entry.setTitle(item.label)
                    .setIcon(item.icon)
                    .setChecked(item.route === this.currentWorkspaceRoute)
                    .onClick(async () => {
                        await this.routeWorkspaceRibbonMenu(item.route);
                    });
            });
        }
    ...
    private async routeWorkspaceRibbonMenu(route: string): Promise<void> {
        const workspaceManager = new WorkspaceManager(this.app);

        if (route === 'home') {
            const instance = (this.app as any).internalPlugins?.getPluginById?.('workspaces')?.instance;
            if (instance?.activeWorkspace) {
                await instance.saveWorkspace(instance.activeWorkspace);
            }

            const defaultLayout = this.defaultWorkspaceLayout ?? this.safeCurrentWorkspaceLayout();
            if (defaultLayout) {
                await this.app.workspace.changeLayout(defaultLayout);
            }
            this.currentWorkspaceRoute = 'home';
            return;
        }

        await workspaceManager.switchWorkspace(route);
        this.currentWorkspaceRoute = route;
    }
    ...
    async onload(): Promise<void> {
        try {
            this.defaultWorkspaceLayout = this.cloneLayout(this.app.workspace.getLayout()) ?? null;
        } catch {
            this.defaultWorkspaceLayout = null;
        }
        this.loadWorkspaceLayouts();

        console.log('[StoryLine Plugin] Test dzialania hot reload');
        ...
        this.registerView(FIREFLY_MAIN_VIEW_TYPE, (leaf) =>
            new FireflyMainView(leaf, this)
        );
        this.registerView(FIREFLY_FILE_BROWSER_VIEW_TYPE, (leaf) =>
            new FireflyFileBrowserView(leaf, this)
        );
        ...
        // Workspace ribbon launcher menu: quick access to six labeled workspace areas.
        this.addRibbonIcon('storyline-logo', 'StoryLine workspace launcher', (evt: MouseEvent) => {
            this.showWorkspaceRibbonMenu(evt);
        });
