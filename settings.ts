...
import type { AreaConfig } from './firefly/models/Area';
import { AREA_ORDER } from './firefly/constants';
...
export class SceneCardsSettingTab extends PluginSettingTab {
    ...
    private renderSettingsTab(containerEl: HTMLElement): void {
        ...
        const fireflyBody = createSettingsSection('Firefly — Obszary');

        const makeAreaConfig = (id: (typeof AREA_ORDER)[number]): AreaConfig => {
            const existing = (this.plugin.settings.areaConfigs ?? []).find(cfg => cfg.id === id);
        ...
        for (const areaId of AREA_ORDER) {
            const area = makeAreaConfig(areaId);
            const areaName = area.label || area.id;
            const section = fireflyBody.createDiv({ cls: 'firefly-area-settings-row' });

            new Setting(section)
                .setName(areaName)
                .setDesc('Folder bazowy i filtry frontmatter dla obszaru')
                .addText(text => {
                    text
                        .setPlaceholder('np. Moja Powieść/Sceny')
                        .setValue(area.rootFolder || '')
                        .onChange(async (value) => {
                            const configs = this.plugin.settings.areaConfigs ?? [];
                            const existing = configs.find(cfg => cfg.id === areaId);
                            const updated: AreaConfig = existing ?? {
                                id: areaId,
                                label: areaName,
                                icon: 'file',
                                rootFolder: '',
                            ...
                            updated.id = areaId;
                            updated.label = areaName;
                            updated.rootFolder = value.trim();
                            if (!existing) configs.push(updated);
                            this.plugin.settings.areaConfigs = configs;
                            await this.plugin.saveSettings();
                        });
                });
            ...
            new Setting(section)
                .setName('Typy plików')
                .setDesc('Lista frontmatter typu pliku, np. Rozdział, Scena')
                .addText(text => {
                    text
                        .setPlaceholder('Rozdział, Scena')
                        .setValue((area.allowedFileTypes ?? []).join(', '))
                        .onChange(async (value) => {
                            const configs = this.plugin.settings.areaConfigs ?? [];
                            const existing = configs.find(cfg => cfg.id === areaId);
                            const updated: AreaConfig = existing ?? {
                            ...
                            updated.id = areaId;
                            updated.label = areaName;
                            updated.allowedFileTypes = value.split(',').map(x => x.trim()).filter(Boolean);
                            if (!existing) configs.push(updated);
                            this.plugin.settings.areaConfigs = configs;
                            await this.plugin.saveSettings();
                        });
            ...
            new Setting(section)
                .setName('Tagi')
                .setDesc('Tagi frontmatter wymagane dla tego obszaru')
                .addText(text => {
                    text
                        .setPlaceholder('pisanie, scena')
                        .setValue((area.allowedTags ?? []).join(', '))
                        .onChange(async (value) => {
                            const configs = this.plugin.settings.areaConfigs ?? [];
                            const existing = configs.find(cfg => cfg.id === areaId);
                            const updated: AreaConfig = existing ?? {
                            ...
                            updated.id = areaId;
                            updated.label = areaName;
                            updated.allowedTags = value.split(',').map(x => x.trim()).filter(Boolean);
                            if (!existing) configs.push(updated);
                            this.plugin.settings.areaConfigs = configs;
                            await this.plugin.saveSettings();
                        });
